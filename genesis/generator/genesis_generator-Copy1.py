...
def st_sampled_softmax(logits, temperature=None):
	with ops.name_scope("STSampledSoftmax") as namescope :
		temp_logits = logits
		if temperature is not None :
			temp_logits = logits / temperature
		nt_probs = tf.nn.softmax(temp_logits)
		onehot_dim = temp_logits.get_shape().as_list()[1]
		sampled_onehot = tf.one_hot(tf.squeeze(tf.multinomial(temp_logits, 1), 1), onehot_dim, 1.0, 0.0)
		with tf.get_default_graph().gradient_override_map({'Ceil': 'Identity', 'Mul': 'STMul'}):
			return tf.ceil(sampled_onehot * nt_probs)

def st_hardmax_softmax(logits, temperature=None):
...
def sample_pwm_only(pwm_logits, temperature=None) :
	n_sequences = K.shape(pwm_logits)[0]
	seq_length = K.shape(pwm_logits)[1]
	
	flat_pwm = K.reshape(pwm_logits, (n_sequences * seq_length, 4))
	sampled_pwm = st_sampled_softmax(flat_pwm, temperature=temperature)

	return K.reshape(sampled_pwm, (n_sequences, seq_length, 4, 1))

def sample_pwm(pwm_logits, temperature=None) :
	n_sequences = K.shape(pwm_logits)[0]
	seq_length = K.shape(pwm_logits)[1]
	
	flat_pwm = K.reshape(pwm_logits, (n_sequences * seq_length, 4))
	sampled_pwm = K.switch(K.learning_phase(), st_sampled_softmax(flat_pwm, temperature=temperature), st_hardmax_softmax(flat_pwm))

	return K.reshape(sampled_pwm, (n_sequences, seq_length, 4, 1))

def sample_gumbel_only(pwm_logits, temperature) :
	n_sequences = K.shape(pwm_logits)[0]
	seq_length = K.shape(pwm_logits)[1]
	
	flat_pwm = K.reshape(pwm_logits, (n_sequences * seq_length, 4))
	sampled_pwm = gumbel_softmax(flat_pwm, temperature)

	return K.reshape(sampled_pwm, (n_sequences, seq_length, 4, 1))

def sample_gumbel(pwm_logits, temperature) :
	n_sequences = K.shape(pwm_logits)[0]
	seq_length = K.shape(pwm_logits)[1]
	
	flat_pwm = K.reshape(pwm_logits, (n_sequences * seq_length, 4))
	sampled_pwm = K.switch(K.learning_phase(), gumbel_softmax(flat_pwm, temperature), st_hardmax_softmax(flat_pwm))
...
def build_generator(batch_size, seq_length, load_generator_function, n_classes=1, n_samples=None, sample_mode='st', sequence_templates=None, batch_normalize_pwm=False, anneal_temp=None, validation_sample_mode='max', supply_inputs=False) :
	...
	if sample_mode == 'st' :
		...
		sample_func = sample_gumbel
		if validation_sample_mode == 'sample' :
			sample_func = sample_gumbel_only
    
	def sample_wrapper(input_tensor, sample_func=sample_func, anneal_temp=anneal_temp) :
		return sample_func(input_tensor, anneal_temp)
	...
	if use_samples :
		...
		#sampled_pwm_1 = Lambda(lambda x, sample_func=sample_func: sample_func(x, anneal_temp), name='pwm_sampler_1')(pwm_logits_upsampled_1)
		sampled_pwm_1 = Lambda(sample_wrapper, name='pwm_sampler_1')(pwm_logits_upsampled_1)
		...
		#sampled_pwm_2 = Lambda(lambda x, sample_func=sample_func: sample_func(x, anneal_temp), name='pwm_sampler_2')(pwm_logits_upsampled_2)
		sampled_pwm_2 = Lambda(sample_wrapper, name='pwm_sampler_2')(pwm_logits_upsampled_2)
...
def get_generator_copier(master_generator, copy_number=1) :
	...
	def copy_generator(batch_size, seq_length, load_generator_function, n_classes=1, n_samples=None, sample_mode='st', sequence_templates=None, batch_normalize_pwm=False, anneal_temp=None, validation_sample_mode='max', supply_inputs=False, master_generator=master_generator, copy_number=copy_number) :
